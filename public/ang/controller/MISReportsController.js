app.controller('MISReportsController', function ($scope, $http, BASE_URL, $ngConfirm, filterFilter) {
	...
	$scope.funGetEmployeeSalesTargetList = function (division_id) {
		if (angular.isDefined(division_id)) {
			$http({
				method: 'GET',
				url: BASE_URL + 'master/employee/get-employee-list/' + division_id
			}).success(function (result) {
				$scope.employeeSalesTargetList = result.employeeListData;
				$scope.clearConsole();
			});
		}
	};
	...
	$scope.funGetCustomerList = function (employee_id) {
		if (employee_id) {
			$http({
				method: 'GET',
				url: BASE_URL + 'MIS/get-customer-list-from-sales-excecutive/' + employee_id,
			}).success(function (result) {
	...
	//**********Viewing of sample log status detail* 21 July,2108************************************************
	$scope.funShowSampleStatusLog = function (postedData) {
		$scope.MISReportDetailData = {};
		$scope.loaderMainShow();
		$http({
			url: BASE_URL + "MIS/view-mis-status-log-detail",
			method: "POST",
			data: { formData: postedData }
		}).success(function (result, status, headers, config) {
			if (result) {
				$scope.MISReportDetailData = result.returnData;
	...
	//**********/Viewing of sample log status detail*********************************************
});