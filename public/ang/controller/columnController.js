app.controller('columnController', function ($scope, $http, BASE_URL, $ngConfirm, $timeout) {
    ...
    //sorting variables
    $scope.sortType = 'column_id';    	     // set the default sort type
    ...
    $scope.generateDefaultCode = function () {
        ...
        $http({
            method: 'GET',
            url: BASE_URL + 'master/column-master/generate-column-code'
        }).success(function (result) {
            $scope.default_column_code = result.uniqueCode;
            $scope.clearConsole();
            $scope.loaderHide();
        });
    };
    ...
    $scope.funListMaster = function () {
        $scope.loaderShow();
        $http({
            url: BASE_URL + "master/column-master/list",
            method: "POST",
    ...
    $scope.funAddMaster = function () {
        ...
        $scope.loaderShow();
        $http({
            url: BASE_URL + "master/column-master/add",
            method: "POST",
    ...
    $scope.funEditMaster = function (id) {
        if (id) {
            ...
            $scope.hideAlertMsg();
            $http({
                url: BASE_URL + "master/column-master/view/" + id,
                method: "GET",
            }).success(function (result, status, headers, config) {
                if (result.error == 1) {
                    $scope.resetButton();
    ...
    $scope.funUpdateMaster = function () {
        ...
        $scope.loaderShow();
        $http({
            url: BASE_URL + "master/column-master/update",
            method: "POST",
            data: { formData: formData }
        }).success(function (result, status, headers, config) {
    ...
    $scope.funDeleteMaster = function (id) {
        if (id) {
            ...
            $scope.hideAlertMsg();
            $http({
                url: BASE_URL + "master/column-master/delete/" + id,
            }).success(function (result, status, headers, config) {
                if (result.error == 1) {
                    $scope.funListMaster();
                    $scope.successMsgShow(result.message);
