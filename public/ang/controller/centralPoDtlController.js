app.controller('centralPoDtlController', function($scope, $http, BASE_URL, $window ,$ngConfirm){
	
	// sorting variables
	$scope.sortType           	= 'cpo_id'; 		//set the default sort type
	$scope.sortReverse  	  	= false; 		//set the default sort order
	$scope.customerGstCategoryList 	= [];
	$scope.editCustomerGstCategory 	= {};
	$scope.centralPODtl 		= {};
	$scope.centralPODtl.cpo_status= '';
	...
	$scope.centralPODtl.cpo_status = {id : 1};
	//*******************/Pre populated Select Box************************
	
	//**********Reset Form Button******************************************
	$scope.resetForm = function(){
		$scope.centralPODtl = {};	
		$scope.erpAddCentralPOForm.$setUntouched();
		$scope.erpAddCentralPOForm.$setPristine();
	...
	$http({
		method: 'POST',
		url: BASE_URL +'master/pos/customers-list'
	...
	$scope.funGetCityOnCustomerChange = function(customer_id){
		if(customer_id){
			$http({
				method: 'GET',
				url: BASE_URL +'master/pos/get-customer-citiy/'+customer_id,
			}).success(function (result) {
				$scope.customerCityList = [{id : result.customerLocationLicNo.city_id,name: result.customerLocationLicNo.city_name}];
				$scope.centralPODtl.cpo_customer_city = {id : result.customerLocationLicNo.city_id};
				$scope.clearConsole();
			});
		}
	}
	...
	$scope.funGetCentralContentListing = function(){
		$http({
			url: BASE_URL + "master/pos/central-po-listing",
			method: "POST",			
		}).success(function (result, status, headers, config) { 
			$scope.centralContentList = result.centralPoContentList;
			$scope.clearConsole(); 
	...
	$scope.funAddCentralContentDtl = function(){
		...
		if(!$scope.erpAddCentralPOForm.$valid)return;
		if($scope.erpAddCentralPOFormFlag)return;
		if(!angular.element("#cpo_file_name").val())return;
		$scope.erpAddCentralPOFormFlag = true;
		formdata.append('formData', $(erpAddCentralPOForm).serialize());
		$scope.loaderShow();
		
		$http({
			url: BASE_URL + "master/pos/add-central-po",
			method: "POST",
			headers: { 'Content-Type': undefined },
			data: formdata,
		}).success(function (result, status, headers, config) {
			$scope.erpAddCentralPOFormFlag = false;
			if(result.error == 1){
				$scope.resetForm();
	...
	$scope.funDeleteCentralPo = function(id){   
		if(!id)return;
		$scope.loaderShow();		
		$http({
			url: BASE_URL + "master/pos/delete-po-dtl",
			method: "POST",
			data: {cpo_id : id},
		}).success(function (result, status, headers, config) {  
			if(result.error == 1){
