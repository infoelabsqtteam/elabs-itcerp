app.controller('orderReportDisciplineController', function($scope, $http, BASE_URL,$ngConfirm,$timeout) {
	...
	$scope.backButton = function(){
		...
		$scope.addMasterFormBladeDiv 	= false;
		$scope.editMasterFormBladeDiv 	= true;
		$scope.addMasterModel.or_discipline_status = {id : 1};
	};
	//**********/Back Button********************************************
	...
	$scope.default_or_discipline_code  = '';
	$scope.generateDefaultCode = function(){
		$scope.loaderShow();
		$http({
			method: 'GET',
			url: BASE_URL +'master/disciplines/generate-auto-code'
		}).success(function (result){
			$scope.default_or_discipline_code = result.uniqueCode;
			$scope.clearConsole();
			$scope.loaderHide();
		});
	};
	...
	$scope.funGetParentCategoryList = function(){
		$http({
			...
			$scope.parentCategoryDropdownList = result.parentCategoryDropdownList;
			$scope.clearConsole();
		});
	};
	//****************/display parent category*********************************
	...
	$scope.funListMaster = function(){
		$scope.loaderShow();
		$http({
			url: BASE_URL + "master/disciplines/list",
			method: "POST",
	...
	$scope.funAddMaster = function(){
		...
		$scope.loaderShow();
		$http({
			url: BASE_URL + "master/disciplines/add",
			method: "POST",
	...
	$scope.funEditMaster = function(id){
		if(id){
			...
			$scope.hideAlertMsg();		
			$http({
				url: BASE_URL + "master/disciplines/view/"+id,
				method: "GET",
			}).success(function(result, status, headers, config){
				if(result.error == 1){
					$scope.resetButton();
					...
					$scope.editMasterModel.or_discipline_status = {id: result.editMasterData.or_discipline_status};
				}else{
	...
	$scope.funUpdateMaster = function(){
		...
		$scope.loaderShow();
		$http({
			url: BASE_URL + "master/disciplines/update",
			method: "POST",
			data: {formData :formData}
		}).success(function (result, status, headers, config) {
	...
	$scope.funDeleteMaster = function(id){				
		if(id){
			...
			$scope.hideAlertMsg();
			$http({
				url: BASE_URL + "master/disciplines/delete/"+id,
			}).success(function(result, status, headers, config){				
				if(result.error == 1){
					$scope.funListMaster();
					$scope.successMsgShow(result.message);					
