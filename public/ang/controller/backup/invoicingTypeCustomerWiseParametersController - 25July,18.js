app.controller('invoicingTypeCustomerWiseParametersController', function($scope, $q, $timeout, $filter, $http, BASE_URL, $ngConfirm) {
	...
	//**********confirm box******************************************************
	$scope.funConfirmDeleteMessage = function(id,cirCustomerID){
		$ngConfirm({
			title     : false,
			...
			buttons   : {
				OK: {
					...
					action: function () {
						$scope.funDeleteCustomerWiseParameterRate(id,cirCustomerID);
					}
				},
				cancel: {
					text     : 'cancel',
	...
	$scope.funGetCustomerWiseParameterRates = function(cirCustomerID,cirDeptId,cirDivisionID){

		$scope.cirCustomerID = cirCustomerID;
		$scope.cirDeptID = !cirDeptId ? 1 : cirDeptId ;
		$scope.cirDivisionID = !cirDivisionID ? 1 : cirDivisionID ;
		$scope.loaderShow();
		var http_para_string = {formData : 'cir_customer_id='+$scope.cirCustomerID+'&'+'dept_id='+$scope.cirDeptID+'&'+'division_id='+$scope.cirDivisionID};
	...
	$scope.funEditSelectedCustomerParametersRate = function(cir_customer_id){
		if(cir_customer_id){
			...
			$scope.loaderMainShow();
			$scope.hideAlertMsg();
			var http_para_string = {formData : 'cir_customer_id='+$scope.cirCustomerID};
	...
	//***************** Adding of Branch Wise DebitNote **********************************
	$scope.funUpdateCustomerWiseParametersRate = function(){
		...
		$http({
			...
			if(data.error == 1){
				...
				$scope.funEditSelectedCustomerParametersRate($scope.cirCustomerID);
	...
	$scope.funDeleteCustomerWiseParameterRate = function(id){
		if(id){
			$scope.loaderMainShow();
			$scope.hideAlertMsg();
			$http({
	...
	$scope.funCustomerWiseParameterAccToDept = function(deptID,divisionID){
		globalDeptID = deptID;
		globalDivisionID =divisionID
		$scope.funGetCustomerWiseParameterRates($scope.cirCustomerID,deptID,divisionID);
	};

	$scope.funGetRefreshCustomerWiseParameterRates = function(cir_id,deptID,divisionID){
		$scope.funGetCustomerWiseParameterRates(cir_id,deptID,divisionID);
