app.controller('orderReportSignDtlController', function ($scope, $http, BASE_URL, $ngConfirm, $timeout) {
	...
	//sorting variables
	$scope.sortType = 'orsd_id';    	 // set the default sort type
	...
	$scope.equipmentCheckboxList = [];
	$http({
		method: 'POST',
		url: BASE_URL + 'equipment-types-list'
	}).success(function (result) {
		if (result.equipmentTypesList) {
			$scope.equipmentCheckboxList = result.equipmentTypesList;
		}
		$scope.clearConsole();
	});
	...
	$scope.employeeSalesTargetList = [];
	$scope.funGetEmployeeBranchWiseList = function (division_id) {
		if (angular.isDefined(division_id)) {
			$http({
				method: 'GET',
				url: BASE_URL + 'master/employee/get-employee-list/' + division_id
			}).success(function (result) {
				$scope.employeeDropdownList = result.employeeListData;
				$scope.clearConsole();
			});
		}
	};
	...
	$scope.funListMaster = function () {
		$scope.loaderShow();
		$http({
			url: BASE_URL + "master/order-report-signatures/list",
			method: "POST",
	...
	$scope.funAddMaster = function () {
		...
		$scope.loaderShow();
		$http({
			url: BASE_URL + "master/order-report-signatures/add",
			method: "POST",
	...
	$scope.funEditMaster = function (id) {
		if (id) {
			...
			$scope.hideAlertMsg();
			$http({
				url: BASE_URL + "master/order-report-signatures/view/" + id,
				method: "GET",
			}).success(function (result, status, headers, config) {
				if (result.error == 1) {
					$scope.resetButton();
					...
					$timeout(function () {
						$scope.employeeDropdownList = [{ id: result.editMasterData.orsd_employee_id, name: result.editMasterData.orsd_employee_name }];
						$scope.divisionDropdownList = [{ id: result.editMasterData.orsd_division_id, name: result.editMasterData.orsd_division_name }];
						$scope.parentCategoryDropdownList = [{ id: result.editMasterData.orsd_product_category_id, name: result.editMasterData.orsd_product_category_name }];
						$scope.editMasterModel.orsd_employee_id = { selectedOption: { id: result.editMasterData.orsd_employee_id } };
						$scope.editMasterModel.orsd_division_id = { selectedOption: { id: result.editMasterData.orsd_division_id } };
						$scope.editMasterModel.orsd_product_category_id = { selectedOption: { id: result.editMasterData.orsd_product_category_id } };
					}, 100);
				} else {
	...
	$scope.funUpdateMaster = function () {
		...
		$scope.loaderShow();
		$http({
			url: BASE_URL + "master/order-report-signatures/update",
			method: "POST",
			data: { formData: formData }
		}).success(function (result, status, headers, config) {
	...
	$scope.funDeleteMaster = function (id) {
		if (id) {
			...
			$scope.hideAlertMsg();
			$http({
				url: BASE_URL + "master/order-report-signatures/delete/" + id,
			}).success(function (result, status, headers, config) {
				if (result.error == 1) {
					$scope.funListMaster();
					$scope.successMsgShow(result.message);
