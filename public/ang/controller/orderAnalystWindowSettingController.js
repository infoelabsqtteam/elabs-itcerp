app.controller('orderAnalystWindowSettingController', function ($scope, $http, BASE_URL, $ngConfirm, $timeout) {
	...
	//sorting variables
	$scope.sortType 	= 'oaws_id';   // set the default sort type
	...
	$scope.funListMaster = function () {
		$scope.loaderShow();
		$http({
			url: BASE_URL + "master/order-analyst-window-settings/list",
			method: "POST",
	...
	$scope.funAddMaster = function () {
		...
		$scope.loaderShow();
		$http({
			url: BASE_URL + "master/order-analyst-window-settings/add",
			method: "POST",
	...
	$scope.funEditMaster = function (id) {
		if (id) {
			...
			$scope.hideAlertMsg();
			$http({
				url: BASE_URL + "master/order-analyst-window-settings/view/" + id,
				method: "GET",
			}).success(function (result, status, headers, config) {
				if (result.error == 1) {
					$scope.resetButton();
					...
					$timeout(function () {
						$scope.divisionDropdownList = [{ id: result.editMasterData.oaws_division_id, name: result.editMasterData.oaws_division_name }];
						$scope.parentCategoryDropdownList = [{ id: result.editMasterData.oaws_product_category_id, name: result.editMasterData.oaws_product_category_name }];
						$scope.editMasterModel.oaws_division_id = { selectedOption: { id: result.editMasterData.oaws_division_id } };
						$scope.editMasterModel.oaws_product_category_id = { selectedOption: { id: result.editMasterData.oaws_product_category_id } };
					}, 100);
				} else {
	...
	$scope.funUpdateMaster = function () {
		...
		$scope.loaderShow();
		$http({
			url: BASE_URL + "master/order-analyst-window-settings/update",
			method: "POST",
			data: { formData: formData }
		}).success(function (result, status, headers, config) {
			$scope.newErpEditMasterFormflag = false;
			if (result.error == 1) {
				$scope.funEditMaster(result.oaws_id);
	...
	$scope.funDeleteMaster = function (id) {
		if (id) {
			...
			$scope.hideAlertMsg();
			$http({
				url: BASE_URL + "master/order-analyst-window-settings/delete/" + id,
			}).success(function (result, status, headers, config) {
				if (result.error == 1) {
					$scope.funListMaster();
					$scope.successMsgShow(result.message);
