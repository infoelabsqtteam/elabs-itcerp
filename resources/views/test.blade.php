               ...
               <td  bgcolor="#FFFFFF">
                  <table class="table table-striped" border="1px solid black" style="border-collapse:collapse;">
                     ...
                     <tr>
                        ...
                        <td><b> Reporting To:	</b><span>{{!empty($stbOrderList->stb_reporting_to) ? $stbOrderList->stb_reporting_to : '-' }}</span></td>
                        <td colspan="2"><b>Invoicing To:</b><span>{{!empty($stbOrderList->stb_invoicing_to) ? $stbOrderList->stb_invoicing_to : '-' }}</span></td>
                     </tr>
                     <tr>
                           ...
                           <table class="table table-striped">
                              ...
                              @foreach($returnData as $key=> $prototypeDetail)
                               <tr  style="background: #e4dede !important;">
										  <td class="padg-2" width="1%"><b>{{$key+1}}.</b></td>
										  <td class="padg-2" width="20%">{{!empty($prototypeDetail->stb_label_name) ? $prototypeDetail->stb_label_name : '-'}}</td>
										  <td class="padg-2" width="10%">{{!empty($prototypeDetail->stb_start_date) ? $prototypeDetail->stb_start_date : '-'}}</td>
										  <td class="padg-2" width="10%">{{!empty($prototypeDetail->stb_end_date) ? $prototypeDetail->stb_end_date : '-'}}</td>
										  <td class="padg-2" width="30%">{{!empty($prototypeDetail->product_name) ? $prototypeDetail->product_name : '-'}}</td>
										  <td class="padg-2" width="10%">{{!empty($prototypeDetail->test_std_name) ? $prototypeDetail->test_std_name : '-'}}</td>
										  <td class="padg-2" width="30%">{{!empty($prototypeDetail->test_code) ? $prototypeDetail->test_code : '-'}}</td>
										</tr>
                              ...
                              <tr>
                                 <td colspan="7">
                                    <table class="table table-striped" border="1px solid black" style="border-collapse:collapse;">
                                       ...
                                       <tr>
                                          <td colspan="9" class="padg-2"><b>{{!empty($parameterCategory) ? $parameterCategory : '' }}</b></td>
                                       </tr>
                                       ...
                                       <tr>
                                          <td colspan="9" class="padg-2">{{!empty($paramDetail->parameters) ? $paramDetail->parameters : '-'}}</td>
                                       </tr>
                                       @else
                                       <tr>
                                          <td class="padg-2 width40">{{!empty($paramDetail->parameters) ?  strip_tags($paramDetail->parameters) : '-'}}</td>
                                          <td class="padg-2 width20">{{!empty($paramDetail->equipment) ? $paramDetail->equipment : '-'}}</td>
                                          <td class="padg-2 width20">{{!empty($paramDetail->method) ? $paramDetail->method : '-'}}</td>
                                          <td class="padg-2 width20" colspan="2">{{!empty($paramDetail->requirement_from_to) ? $paramDetail->requirement_from_to : '-'}}</td>
                                       </tr>
                                       @endif
                                       @endforeach	
                                       @endforeach	
