...
<div id="header">	
	...
	<div class="header-inv-content"@if(!empty($hasContainHeaderFooter) && $hasContainHeaderFooter == '2')style="visibility: hidden;"@endif>
		<?php echo !empty($viewData['debitNoteHeader']['header_content']) ? htmlspecialchars_decode($viewData['debitNoteHeader']['header_content']) : '';?>
	</div>
	
	<div style="font-size:15px!important;text-align:center!important;font-weight:bold;">Debit Note</div>
</div>
<!--- /header-->

<!--- footer-->
<div id="footer" class="footer-inv-content">
	<?php echo !empty($viewData['debitNoteHeader']['footer_content']) ? htmlspecialchars_decode($viewData['debitNoteHeader']['footer_content']) : '';?>
</div>
<!--- /footer-->
				...
				<tbody>
					<tr>
						<td colspan="3" rowspan="2">
							...
							<p style="font-weight:500;margin-left: 15px;">{{$viewData['debitNoteHeader']['customer_name']}}</p>
							<p style="margin-left:15px;"><b>Billing Address</b></p>
							<p style="margin-left:15px;">{{$viewData['debitNoteHeader']['customer_address']}}</p>
							<p style="font-weight:500;margin-left: 15px;">{{$viewData['debitNoteHeader']['customer_city_name']}}<span style="float:right">{{$viewData['debitNoteHeader']['customer_state_name']}}</span></p>
							<p style="font-weight:500;margin-left: 15px;">{{ defined('GSTIN') && !empty(GSTIN) ? GSTIN : 'GSTIN:'}}{{$viewData['debitNoteHeader']['customer_gst_no']}}</p>
						</td>
						<th colspan="2" align="left">
							<h4 style="margin: 0px;"><strong>Debit Note No.:&nbsp;</strong><span style="font-weight:normal!important;">{{$viewData['debitNoteHeader']['debit_note_no']}}</span></h4>
							<h4 style="margin: 0px;"><strong>Debit Note Date:&nbsp;</strong><span style="font-weight:normal!important;">{{date('d-m-Y',strtotime($viewData['debitNoteHeader']['debit_note_date']))}}</span></h4>
						</th>
					</tr>
					<tr>
						<th colspan="2" align="left">
							<p><strong>Reference No./Invoice No.</strong></p>
							@if(!empty($viewData['debitNoteHeader']['invoice_no']))
							<h4 style="margin:0px;"><span style="font-weight:normal!important;">{{$viewData['debitNoteHeader']['invoice_no']}}</span></h4>
							@endif
							@if(!empty($viewData['debitNoteHeader']['invoice_date']))
							<h4 style="margin:0px;"><strong>Dated:&nbsp;</strong><span style="font-weight:normal!important;">{{date('d-m-Y',strtotime($viewData['debitNoteHeader']['invoice_date']))}}</span></h4>
							@endif
						</th>
					</tr>
					...
					@if(!empty($viewData['debitNoteBody']))
						@if(!empty($viewData['debitNoteHeader']['billing_type']) && $viewData['debitNoteHeader']['billing_type'] == '5')
							@foreach($viewData['debitNoteBody'] as $PONumberKey => $invoiceBodyPoAll)
								<tr>
							...
							@endforeach
						@else
							@foreach($viewData['debitNoteBody'] as $key => $invoiceBody)
								<tr>
									<td align="center" style="font-size:12px!important;overflow-wrap:break-word;word-wrap: break-word;">{{$key+1}}</td>
									<td align="left" style="font-size:12px!important;overflow-wrap:break-word;word-wrap: break-word;">{{$invoiceBody['name_of_product']}}</td>
									<td align="left" style="font-size:12px!important;overflow-wrap:break-word;word-wrap: break-word;">{{$invoiceBody['batch_no']}}</td>
					...
					<tr>
						<td colspan="2" style="vertical-align: top;">
							...
							<p>{{ucwords($viewData['debitNoteFooter']['net_total_in_words'])}}</p>
						</td>
						<td colspan="2" style="text-align:right!important">
							<h5 style="padding: 5px; margin: 0px ! important;"><b>Total</b></h5>
							@if(!empty(round($viewData['debitNoteFooter']['discount'])))
							<h5 style="padding: 5px; margin: 0px ! important;">
							    <b>Discount</b>
							    @if(!empty($viewData['debitNoteFooter']['discount_text']))
							    <b>{{$viewData['debitNoteFooter']['discount_text']}}</b>
							    @endif
							</h5>
							@endif
							@if(!empty(round($viewData['debitNoteFooter']['net_amount'])))
							<h5 style="padding: 5px; margin: 0px ! important;"><b>Net Total</b></h5>
							@endif
							@if(!empty(round($viewData['debitNoteFooter']['surcharge_amount'])))
							<h5 style="padding: 5px; margin: 0px ! important;"><b>Surcharge Amount</b></h5>
							@endif
							@if(!empty(round($viewData['debitNoteFooter']['extra_amount'])))
							<h5 style="padding: 5px; margin: 0px ! important;"><b>Convenience & Sampling Amount</b></h5>
							@endif
							@if(!empty($viewData['debitNoteFooter']['sgst_rate']) && !empty($viewData['debitNoteFooter']['sgst_amount']))
							<h5 style="padding: 5px; margin: 0px ! important;"><b>State Goods & Service Tax ({{$viewData['debitNoteFooter']['sgst_rate']}}%)</b></h5>
							@endif
							@if(!empty($viewData['debitNoteFooter']['cgst_rate']) && !empty($viewData['debitNoteFooter']['cgst_amount']))
							<h5 style="padding: 5px; margin: 0px ! important;"><b>Central Goods & Service Tax ({{$viewData['debitNoteFooter']['cgst_rate']}}%)</b></h5>
							@endif
							@if(!empty($viewData['debitNoteFooter']['igst_rate']) && !empty($viewData['debitNoteFooter']['igst_amount']))
							<h5 style="padding: 5px; margin: 0px ! important;"><b>Integrated Goods & Service Tax ({{$viewData['debitNoteFooter']['igst_rate']}}%)</b></h5>
							@endif
						</td>
						<td>
							<h5 style="padding: 5px; margin: 0px ! important;text-align:right!important;"><b>{{$viewData['debitNoteFooter']['total']}}</b></h5>
							@if(!empty(round($viewData['debitNoteFooter']['discount'])))
							<h5 style="padding: 5px; margin: 0px ! important;text-align:right!important;"><b>{{$viewData['debitNoteFooter']['discount']}}</b></h5>
							@endif
							@if(!empty(round($viewData['debitNoteFooter']['net_amount'])))
							<h5 style="padding: 5px; margin: 0px ! important;text-align:right!important;"><b>{{$viewData['debitNoteFooter']['net_amount']}}</b></h5>
							@endif
							@if(!empty(round($viewData['debitNoteFooter']['surcharge_amount'])))
							<h5 style="padding: 5px; margin: 0px ! important;text-align:right!important;"><b>{{$viewData['debitNoteFooter']['surcharge_amount']}}</b></h5>
							@endif
							@if(!empty(round($viewData['debitNoteFooter']['extra_amount'])))
							<h5 style="padding: 5px; margin: 0px ! important;text-align:right!important;"><b>{{$viewData['debitNoteFooter']['extra_amount']}}</b></h5>
							@endif
							@if(!empty($viewData['debitNoteFooter']['sgst_rate']) && !empty($viewData['debitNoteFooter']['sgst_amount']))
							<h5 style="padding: 5px; margin: 0px ! important;text-align:right!important;"><b>{{$viewData['debitNoteFooter']['sgst_amount']}}</b></h5>
							@endif
							@if(!empty($viewData['debitNoteFooter']['cgst_rate']) && !empty($viewData['debitNoteFooter']['cgst_amount']))
							<h5 style="padding: 5px; margin: 0px ! important;text-align:right!important;"><b>{{$viewData['debitNoteFooter']['cgst_amount']}}</b></h5>
							@endif
							@if(!empty($viewData['debitNoteFooter']['igst_rate']) && !empty($viewData['debitNoteFooter']['igst_amount']))
							<h5 style="padding: 5px; margin: 0px ! important;text-align:right!important;"><b>{{$viewData['debitNoteFooter']['igst_amount']}}</b></h5>
							@endif
						</td>
					</tr>
					...
					<tr>
						<td colspan="2">
							@if(!empty($viewData['debitNoteHeader']['debit_note_remark']))
							<p style="font-size:13px!important;font-weight:bold;">Remarks for Debit Note: </p>
							<div class="break-word" style="margin-top:5px!important;font-size:12px!important;">{{$viewData['debitNoteHeader']['debit_note_remark']}}</div>
							@endif
						</td>
						...
						<td align="right" colspan="1">
							<b style="text-align:right!important;"><b>{{$viewData['debitNoteFooter']['net_total']}}</b>
						</td>
					</tr>
					...
					<tr>   
						...
						<td colspan="3" style="vertical-align: bottom;border:1px solid;border-top:0px!important" valign="bottom">
							<div style="font-size:10px;padding-top:-5px!important;float:right!important;">
								@if(!empty($viewData['debitNoteHeader']['user_signature']) && file_exists(DOC_ROOT.SIGN_PATH.$viewData['debitNoteHeader']['user_signature']))
								<p style="float:right;width:100%;@if(!empty($hasContainHeaderFooter) && $hasContainHeaderFooter == '3')visibility:hidden;@endif"><img style="width: 70px; height: 30px; padding-left:30px;" alt="{{$viewData['debitNoteHeader']['invoice_by']}}" src="{{DOC_ROOT.SIGN_PATH.$viewData['debitNoteHeader']['user_signature']}}"/></p>
								@endif
