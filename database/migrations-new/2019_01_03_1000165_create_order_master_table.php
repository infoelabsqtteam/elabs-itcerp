...
class CreateOrderMasterTable extends Migration
{
	...
	public function up()
	{
		Schema::create('order_master', function (Blueprint $table) {
			...
			$table->tinyInteger('order_sample_type')->comment('1 for inter-laboratory,2 for Compensatory')->nullable();
			$table->text('hold_reason')->nullable();
			$table->string('job_order_file')->nullable();
			$table->string('job_analytical_sheet_file')->nullable();
			$table->string('job_analytical_sheet_cal_file')->nullable();
			$table->integer('order_reinvoiced_count')->nullable();
			$table->tinyInteger('tat_in_days')->comment('Entered tat in days')->nullable();
			$table->integer('stb_order_hdr_detail_id')->unsigned()->index()->nullable();
			...
			$table->foreign('reporting_to')->references('customer_id')->on('customer_master');
			$table->foreign('invoicing_to')->references('customer_id')->on('customer_master');			
