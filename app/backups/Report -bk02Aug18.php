...
class Report extends Model
{
	    ...
	    *************************/	    
	    ...
	    public function resetOrderInchargeDetail($order_id){
			...
			//echo '<pre>';print_r($oidArrayKeyIds);print_r($order_id);die;
			
			if(!empty($oidArrayKeyIds) && !empty($order_id)){
				    $dataSave = array('order_incharge_dtl.oid_status' => '0','order_incharge_dtl.oid_confirm_date' => CURRENTDATETIME,'order_incharge_dtl.oid_confirm_by' => $user_id);
	    ...
	    ***
	    ******/
	    public function updateOrderSectionInchargeDetail($orderId,$analysisIdsArr){
			//echo'<pre>'; print_r($analysisIdsArr); die;
			global $order,$models,$report;
			
			$error    = '0';
			$message  = config('messages.message.error');
			$flag     = '0';
			$formData = $equipmentIds = $combinedArr = $commonEmpArr = $newEmpArr = $removedEmpArr = array();
			$inserted = '';
			...
			if(!empty($getEquipmentAndOrderIds)){				
				    ...
				    $orderInchargeId = DB::table('order_incharge_process_dtl')->whereIn('order_incharge_process_dtl.oipd_analysis_id',$analysisIdsArr)->where('oipd_order_id',$orderId)->pluck('oipd_incharge_id')->all();
				    //print_r($orderInchargeId); die;
				    if(!empty($alreadyExist)){
						...
						//die;
						if($removedEmpArr){
							    DB::table('order_incharge_dtl')->where('order_incharge_dtl.order_id',$orderId)->whereIn('order_incharge_dtl.oid_employee_id',$removedEmpArr)->whereIn('order_incharge_dtl.oid_id',$orderInchargeId)->update(['order_incharge_dtl.oid_status' => '2']);  
						}
						if($commonEmpArr){
							    DB::table('order_incharge_dtl')->where('order_incharge_dtl.order_id',$orderId)->whereIn('order_incharge_dtl.oid_employee_id',$commonEmpArr)->whereIn('order_incharge_dtl.oid_id',$orderInchargeId)->update(['order_incharge_dtl.created_at' => CURRENTDATETIME,'order_incharge_dtl.oid_confirm_date' => NULL,'order_incharge_dtl.oid_confirm_by' => NULL]);
						}
						if($newEmpArr){							
