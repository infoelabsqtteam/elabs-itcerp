...
class StandardWiseProductTestController extends Controller
{
	...
	{
		if (!empty($test_code)) {
			$data = DB::table('product_test_hdr')->where('product_test_hdr.test_code', '=', $test_code)->first();
	...
	public function getProductTestList(Request $request, $p_category_id)
	{
		...
		if (!empty($request['keyword']) && strlen($request['keyword']) > 1) {
			...
			$testParametersObj->where(function ($testParametersObj) use ($models, $keyword) {
				return $testParametersObj->where('product_test_hdr.test_code', 'LIKE', '%' . $keyword . '%')
					...
					->Orwhere('product_categories.p_category_name', 'LIKE', '%' . $keyword . '%')
			...
			$testParametersObj->whereMonth('product_test_hdr.created_at', date('m'));
			$testParametersObj->whereYear('product_test_hdr.created_at', date('Y'));
		}
		...
		$testParameters = $testParametersObj->where('product_test_hdr.status', 1)->get();
	...
	public function getProductTestListMultisearch(Request $request)
	{
		...
		if (!empty($searchArry['search_status'])) {
			$testParametersObj->where('product_test_hdr.status', $searchArry['search_status']);
		}
		...
		if ($request->isMethod('post')) {
			if (!empty($request['data']['id'])) {
				$testStandardData = DB::table('product_test_hdr')
					...
					->join('test_standard', 'test_standard.test_std_id', 'product_test_hdr.test_standard_id')
					->select('product_test_hdr.*', 'product_master.*', 'test_standard.*', 'product_categories.p_category_name')
					->where('product_test_hdr.test_id', '=', $request['data']['id'])
					->first();
				if ($testStandardData->test_id) {
					$returnData = array('responseData' => $testStandardData);
				...
				if (!empty($productTestData['data'])) {
					foreach ($productTestData['data'] as $key => $data) {
						...
						if (!empty($data[4]) && strtolower($data[4]) == 'yes') {
							$proData[$key]['claim_dependent'] = 1;
						} else {
							$proData[$key]['claim_dependent'] = 0;
						}
						if (!empty($data[5]) && strtolower($data[5]) == 'numeric') {
							$proData[$key]['standard_value_type'] = 'numeric';
						} else {
							$proData[$key]['standard_value_type'] = 'alphanumeric';
						}
	...
	/***************************************
	 * Description : Cloning of Test Master
	...
	public function saveMoreProductTest(Request $request)
	{
		...
		$error = '0';
		$message = config('messages.message.error');
		$newPostData = $formData = $categoryWiseParamenter = $cloneProductTestHdr = $cloneProductTestHdrDtl = array();
		...
		try {
			...
			if (!empty($request['data']['formData'])) {
				...
				} elseif (empty($formData['test_standard_id'])) {
					$message = config('messages.message.testStdIdRequired');
				} elseif (empty($formData['test_parameters'])) {
					$message = config('messages.message.testParameterRequired');
				} else {
					//Filtering Product Test Dtl Ids Array
					$testParameterIdArray = !empty($formData['test_parameters']) ?  array_filter($formData['test_parameters']) : array();

					//Saving of data in product_test_hdr
					$cloneProductTestHdr['product_id']		= $formData['product_id'];
					$cloneProductTestHdr['test_standard_id']	= $formData['test_standard_id'];
					$cloneProductTestHdr['test_code']		= $standardWiseProductTest->getAutoGeneratedCode($formData['product_id'], $formData['test_standard_id']);
					$cloneProductTestHdr['created_by']   	= USERID;
					$cloneProductTestHdr['status']		= '1';
					$formData['test_id'] 			= DB::table('product_test_hdr')->insertGetId($cloneProductTestHdr);

					//Saving of Data in product_test_dtl table
					if (!empty($formData['test_id']) && !empty($testParameterIdArray)) {
						$productTestDtlDataList = DB::table('product_test_dtl')->whereIn('product_test_dtl.product_test_dtl_id', $testParameterIdArray)->orderBy('product_test_dtl.parameter_sort_by', 'asc')->get()->toArray();
						if (!empty($productTestDtlDataList)) {
							foreach ($productTestDtlDataList as $key => $valuesAll) {
								if (is_object($valuesAll)) {
									$valuesAll->test_id = $formData['test_id'];
									$valuesAll->created_by = USERID;
									foreach ($valuesAll as $columName => $columValue) {
										if ($columName != 'product_test_dtl_id' && $columName != 'created_at' && $columName != 'updated_at') {
											$cloneProductTestHdrDtl[$key][$columName] = $columValue;
										}
									}
								}
							}
						}
						if (!empty($cloneProductTestHdrDtl)) {

							//Saving of Data in product_test_dtl table
							DB::table('product_test_dtl')->insert($cloneProductTestHdrDtl);

							//Updating Test Code
							$standardWiseProductTest->updateProductTestCode($formData);
