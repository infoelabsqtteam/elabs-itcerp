...
class EmployeeController extends Controller
{
    ...
    */
	...
	if(!empty($employeeBanchWiseList)){
		...
		$employeeList[$key]['departments']=DB::table('users_department_detail')
		    ...
		    ->get()
		    ->toArray();			
		$employeeList[$key]['roles']=DB::table('role_user')
		    ->join('roles', 'roles.id', '=', 'role_user.role_id')
		    ->select('role_user.user_id','role_user.role_id','roles.name')
		    ->where('role_user.user_id','=',$user->id)
		    ->get()
		    ->toArray();			
		$employeeList[$key]['equipmentType'] = DB::table('users_equipment_detail')
    ...
    */
	...
	if(!empty($request->formData) && $request->isMethod('post')){
	    ...
	    }else if(!isset($formData['is_sales_person']) && empty($formData['department_id'])){
		$message = config('messages.message.departmentRequired');
	    }else if(!isset($formData['is_sales_person']) && empty($formData['role_id'])){
		$message = config('messages.message.roleRequired');
	    }else if(!isset($formData['is_sales_person']) && in_array('6',$formData['role_id']) && empty($formData['equipment_type_id'])){
		$message = config('messages.message.equipmentTypeRequired');
	    }else if(!empty($formData['role_id']) && in_array('15',$formData['role_id']) && $user->checkMultipleMicrobiologist($formData)){
		$message = config('messages.message.microbiologistExistError');
	    }else{
		...
		$formData 		     = $models->unsetFormDataVariables($formData,array('_token','password_confirmation','department_id','role_id','equipment_type_id'));
    ...
    */
	...
	$message    = config('messages.message.error');
	$data       = '';
	$users = $departmentDetail = $rolesDetail = $equipmentDetail = array();
	
	 if($request->isMethod('post') && !empty($request['data']['user_id'])){
		  $users = DB::table('users')					
			  ->select('users.id','users.division_id', 'users.name', 'users.email', 'users.user_code', 'users.is_sales_person','users.status')
			  ->where('users.id', '=', $request['data']['user_id'])
			  ->first();
		  if(!empty($users->id)){
		 $error       	  = '1';
		 $message     	  = '';
		 $departmentDetail = DB::table('users_department_detail')->where('user_id','=',$users->id)->pluck('department_id');
		 $rolesDetail      = DB::table('role_user')->where('user_id','=',$users->id)->pluck('role_id');
		 $equipmentDetail  = DB::table('users_equipment_detail')->where('user_id','=',$users->id)->pluck('equipment_type_id');
	 ...
	 //echo'<pre>'; print_R($rolesDetail);die;
	 return response()->json(['error'=> $error,'message'=> $message,'data'=> $data,'userData' => $users,'deptDetail'=>$departmentDetail,'rolesDetail'=>$rolesDetail,'equipDetail'=>$equipmentDetail]);
    }
	...
	if(!empty($request->formData) && $request->isMethod('post')){
	    ...
	    $userId = !empty($formData['user_id']) ? $formData['user_id']: '0';
		  ...
		  if(empty($formData['division_id'])){
			  ...
			  $message = config('messages.message.equipmentTypeRequired');
		  }else if(!empty($formData['role_id']) && in_array('15',$formData['role_id']) && $user->checkMultipleMicrobiologist($formData) > 0){
			  $message = config('messages.message.microbiologistExistError');
		  }else{				
			  ...
			  if(!empty($formData['password'])){
					$formData['password'] = bcrypt($formData['password']);
					$unsetUnRequiredColums = array('_token','password_confirmation','department_id','role_id','equipment_type_id','user_id');
			  }else{
					$unsetUnRequiredColums = array('_token','password_confirmation','department_id','role_id','equipment_type_id','user_id','password');
			  }
	...
	public function getDivisionId($division_code){
		if(!empty($division_code)){
			$division = DB::table('divisions')
							->where('divisions.division_code','=',$division_code)
							->first(); 
    ...
    {   
		...
		if ($request->isMethod('post')){
			if(!empty($request['data']['formData'])){  
				...
				{ 	
					if(strtolower($newPostData['is_sales_person'])=='yes'){ $detailsData['is_sales_person'] = 1; }else{ $detailsData['is_sales_person'] = 0; }
    ...
    */
	...
	if($request->isMethod('post') && !empty($request->user_id) && !empty($request->file('user_signature'))){
	    list($signImage,$srcPath) = $user->uploadSignature($request->user_id, $request);
	    if($signImage){
	...
	$error    = '0';
		...
		if(!empty($itemData->user_signature)){
			$srcPath = $user->removeUploadedSignature($user_id, $itemData->user_signature);
		}				
