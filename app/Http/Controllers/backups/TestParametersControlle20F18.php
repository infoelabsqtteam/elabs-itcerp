    ...
    public function createTestParameters(Request $request){
		...
		if ($request->isMethod('post') && !empty($request['data']['formData'])) {
			...
			if(empty($newPostData['test_parameter_code'])){
				...
				if(in_array(strtolower(strip_tags($newPostData['test_parameter_name'])),$parameterDescArr)){
					$newPostData['test_parameter_name'] = strip_tags($newPostData['test_parameter_name']);
				}
