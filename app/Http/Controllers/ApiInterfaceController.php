...
class ApiInterfaceController extends Controller
{
    ...
    **********************************************/
    ...
    //Get list of methods list
    public function getWebRow(Request $request){
		$returnData = array();
		if(!empty($request->tableName) && !empty($request->columnName) && !empty($request->columnValue)){
			$returnData = DB::table($request->tableName)->where($request->columnName,$request->columnValue)->first();
		}
		return response()->json($returnData);
    }
	
	//Get list of methods list
    public function getWebRows(Request $request){
		$returnData = array();
		if(!empty($request->tableName) && !empty($request->columnName) && !empty($request->columnValue) && !empty($request->sortByColumnName) && !empty($request->sortByColumnValue)){
			$returnData = DB::table($request->tableName)->where($request->columnName,$request->columnValue)->orderBy($request->sortByColumnName,$request->sortByColumnValue)->get()->toArray();
		}
		return response()->json($returnData);
    }
    
    //Get list of methods list
    public function getWebCustomersList(Request $request){
		
		$customersList = array();
		
		if(!empty($request->keyword)){
			$keyword = trim(urldecode($request->keyword));
			$customersObj= DB::table('customer_master')->join('city_db','city_db.city_id','customer_master.customer_city');
			$customersObj->where('customer_master.customer_name', 'LIKE', "%$keyword%");
			$customersObj->select('customer_master.customer_code as id',DB::raw('CONCAT(customer_master.customer_code,"/",customer_master.customer_name,"/",city_db.city_name) AS name'));
			$customersObj->orderBy('customer_name','desc');
			$customersObj->limit(100);
			$customersList = $customersObj->get()->toArray();
		}
	...
	//Get list of methods list
    public function getWebRelatedCustomersList(Request $request){
		
		$relatedCustomersList = array();
		
		if(!empty($request->keyword)){
			$keyword = trim(urldecode($request->keyword));
			$relatedCustomerObj= DB::table('customer_master')->join('city_db','city_db.city_id','customer_master.customer_city');
			$relatedCustomerObj->where('customer_master.customer_name', 'LIKE', "%$keyword%");
			$relatedCustomerObj->select('customer_master.customer_code as id',DB::raw('CONCAT(customer_master.customer_code,"/",customer_master.customer_name,"/",city_db.city_name) AS name'));
			$relatedCustomerObj->orderBy('customer_name','desc');
			$relatedCustomersList = $relatedCustomerObj->get()->toArray();
		}
		
		return response()->json($relatedCustomersList);
    }
    ...
    * @return \Illuminate\Http\Response
    */
    public function getWebCustomerDetail(Request $request){
	
		$customerList = array();
		
		//Saving record in orders table
		if(!empty($request->customer_code)){
			$customerList = DB::table('customer_master')
					->join('customer_contact_persons','customer_contact_persons.customer_id','=','customer_master.customer_id')
					->join('city_db','city_db.city_id','=','customer_master.customer_city')
					->where('customer_master.customer_code','=',trim($request->customer_code))
					->select('customer_master.customer_id','customer_master.customer_name','customer_master.customer_email','customer_master.customer_mobile','customer_master.customer_address','customer_contact_persons.contact_name1 as contact_person')
					->first();
		}
	...
	//Get list of divisions code
    public function getWebDivisionsCodeList(){
		$divisionCodeList = DB::table('divisions')->select('division_id as id','division_name as name')->get()->toArray();
		return response()->json($divisionCodeList);
    }
	
	//Get list of items code  list
    public function getWebParentProductCategory(){
		$data = DB::table('product_categories')
				->select('p_category_id as id','p_category_name as name')
				->where('product_categories.level','0')
				->orderBy('product_categories.p_category_id','ASC')
	...
	//Get list of items code  list
    public function getWebStandardProductCategory(Request $request){
		
		if(!empty($request['proCatId'])){
			
			//Getting Selected Product Category ID
			$levelOneProdCatList = DB::table('product_categories')
				->select('p_category_id as id','p_category_name as name')
				->where('product_categories.parent_id',$request['proCatId'])
				->orderBy('product_categories.p_category_id','ASC')
				->get();
			
			//Getting Selected Test Standard
			$testStandardsList = DB::table('test_standard')
				->select('test_std_id as id','test_std_name as name')
				->where('test_standard.product_category_id','=',$request['proCatId'])
				->orderBy('test_standard.test_std_name','ASC')
				->get()
				->toArray();
				
			return response()->json(['testStandardsList' => $testStandardsList,'levelOneProdCatList' => $levelOneProdCatList]);
		}
    }
	
	//Get list of items code  list
    public function getWebSubProductCategory(Request $request){
		
		if(!empty($request['pCatId'])){
			
			//Getting Selected Product Category ID
			$levelTwoProdCatList = DB::table('product_categories')
				->select('p_category_id as id','p_category_name as name')
				->where('product_categories.parent_id',$request['pCatId'])
				->orderBy('product_categories.p_category_id','ASC')
				->get()
				->toArray();
				
			return response()->json(['levelTwoProdCatList' => $levelTwoProdCatList]);
		}
    }
	
	//Get list of items code  list
    public function getWebProductNameList(Request $request){
		
		if(!empty($request['p_category_id']) && !empty($request['test_standard_id'])){
			
			//Getting Selected Product Category ID
			$productNameList = DB::table('product_master')
						->join('product_test_hdr','product_test_hdr.product_id','product_master.product_id')
						->select('product_master.product_id as id','product_master.product_name as name')
						->where('product_master.p_category_id','=',$request['p_category_id'])
						->where('product_test_hdr.test_standard_id','=',$request['test_standard_id'])
						->orderBy('product_master.product_name','ASC')
						->get()
						->toArray();
				
			return response()->json(['productNameList' => $productNameList]);
		}
    }
	
    //Get list of divisions code
    public function getWebTrfStorageConditionList(){
	$trfStorageConditionList = DB::table('trf_storge_condition_dtls')->select('trf_sc_id as id','trf_sc_name as name')->get()->toArray();
	return response()->json($trfStorageConditionList);
    }
	
    //Get list of items code  list
    public function getWebProductTestMasterList(Request $request){
		
		if(!empty($request['product_id']) && !empty($request['test_standard_id'])){
			
			//Getting Selected Product Category ID
			$productTestParameterList = DB::table('product_test_hdr')
				->join('product_test_dtl','product_test_dtl.test_id','product_test_hdr.test_id')
				->join('test_parameter','test_parameter.test_parameter_id','product_test_dtl.test_parameter_id')
				->select('product_test_dtl.product_test_dtl_id','product_test_dtl.test_id','product_test_dtl.test_parameter_id','test_parameter.test_parameter_name')
				->where('product_test_hdr.product_id','=',$request['product_id'])
				->where('product_test_hdr.test_standard_id','=',$request['test_standard_id'])
				->orderBy('product_test_dtl.parameter_sort_by','ASC')
				->get()
				->toArray();
			
			$productTestMasterData = DB::table('product_test_hdr')->where('product_test_hdr.product_id','=',$request['product_id'])->where('product_test_hdr.test_standard_id','=',$request['test_standard_id'])->first();
				
			return response()->json(['productTestParameterList' => $productTestParameterList,'productTestMasterData' => $productTestMasterData]);
		}
    }
	
    //Get list of divisions code
    public function createTrfInErp(Request $request){	

	$error    = '0';
	$formData = $trfHdfDataErp = $trfHdfDataDetailErp = array();
	...
	try {            
		...
		//Assigning form Data to array variable
		$formdata = $request->all();
		
		if(!empty($formdata['trfHdr']) && !empty($formdata['trfHdrDtl'])){
			
		    $trfHdfDataErp = $formdata['trfHdr'];
		    $trfHdfDataDetailErp = $formdata['trfHdrDtl'];
		    
		    //Getting ERP Customer Detail
		    $customerData = DB::table('customer_master')->where('customer_master.customer_code', '=', !empty($trfHdfDataErp['trf_customer_id']) ? $trfHdfDataErp['trf_customer_id'] : '0')->first();
		    $trfHdfDataErp['trf_customer_id'] = !empty($customerData->customer_id) ? trim($customerData->customer_id) : '0';				
		    
		    //TRF Hdr Data
		    $trf_hdr_id = DB::table('trf_hdrs')->insertGetId($trfHdfDataErp);
		    
		    //TRF Hdr Detail Data
		    foreach($trfHdfDataDetailErp as $key => $values){$trfHdfDataDetailErp[$key]['trf_hdr_id'] = $trf_hdr_id;}
		    DB::table('trf_hdr_dtls')->insert($trfHdfDataDetailErp);
    ...
    //Get list of divisions code
    public function updateTrfInErp(Request $request){
	...
	try {            
		...
		if(!empty($formdata['trfNo']) && !empty($formdata['trfHdr']) && !empty($formdata['trfHdrDtl'])){
			
			$trfData = DB::table('trf_hdrs')->where('trf_hdrs.trf_no',$formdata['trfNo'])->first();
			
			if(!empty($trfData->trf_id)){
			
				$trfId = $trfData->trf_id;
				$trfHdfDataErp = $formdata['trfHdr'];
				$trfHdfDataDetailErp = $formdata['trfHdrDtl'];			
				
				//TRF Hdr Data
				DB::table('trf_hdrs')->where('trf_hdrs.trf_id',$trfId)->update($trfHdfDataErp);
				
				//TRF Hdr Detail Data
				if(DB::table('trf_hdr_dtls')->where('trf_hdr_dtls.trf_hdr_id',$trfId)->delete()){
					
					//Assigning TRF Detail
					foreach($trfHdfDataDetailErp as $key => $values){$trfHdfDataDetailErp[$key]['trf_hdr_id'] = $trfId;}
					DB::table('trf_hdr_dtls')->insert($trfHdfDataDetailErp);
    ...
    //Get list of divisions code
    public function updateTrfStatusInErp(Request $request){	
	...
	try {            
		...
		if(!empty($formdata['trf_no']) && !empty($formdata['trf_status'])){
		    
		    DB::table('trf_hdrs')->where('trf_hdrs.trf_no',$formdata['trf_no'])->update(['trf_hdrs.trf_active_deactive_status' => $formdata['trf_status']]); 
                    $error = '1';
    ...
    //Get list of divisions code
    public function getBrandTypeListInErp(){
	$orderBrandTypeList = DB::table('order_brand_type_dtl')->select('order_brand_type_dtl.obtd_id as id','order_brand_type_dtl.obtd_name as name')->get()->toArray();
	return response()->json($orderBrandTypeList);
    }
